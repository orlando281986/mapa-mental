...
import GrowingEdge from "../components/GrowingEdge";
...
function EditorInner() {
  ...
  const [inheritStyle, setInheritStyle] = useState(() => localStorage.getItem("inheritStyle") === "true");
  const [showMiniMap, setShowMiniMap] = useState(true);
  const [toolbarCollapsed, setToolbarCollapsed] = useState(() => localStorage.getItem("toolbarCollapsed") === "true");
  ...
  const edgeTypes = useMemo(() => ({ growing: GrowingEdge }), []);
    ...
    (async () => {
      try {
        ...
        (data.nodes || []).forEach((n) => {
          if (n.data?.dueDate) {
            const diff = Math.ceil((new Date(n.data.dueDate) - now) / (1000 * 60 * 60 * 24));
            if (diff < 0) overdue++;
            else if (diff <= 7) upcoming++;
          }
        });
        if (overdue && upcoming) toast.warning(`${overdue} nó(s) atrasado(s) e ${upcoming} com prazo próximo`);
        else if (overdue) toast.error(`${overdue} nó(s) atrasado(s)!`);
        else if (upcoming) toast.info(`${upcoming} nó(s) com prazo próximo`);
      } catch {
      ...
      try {
        const { data: mapsData } = await api.get("/maps");
        setUserMaps(mapsData.filter((m) => m.id !== id));
      } catch {
        // lista de mapas é opcional
      }
    })();
  }, [id]);
  ...
  const onConnect = useCallback((params) => {
    ...
    setEdges((eds) => addEdge({ ...params, id: edgeId, type: "growing" }, eds));
    setTimeout(() => {
      setEdges((eds) => eds.map((e) =>
        e.id === edgeId ? { ...e, type: "smoothstep", style: undefined } : e
      ));
    }, 1500);
  ...
  const onConnectEnd = useCallback((event) => {
    ...
    const newId = addNodeAt(pos.x, pos.y, inheritStyle ? {
      color: fromNode.data.color,
      shape: fromNode.data.shape,
      textColor: fromNode.data.textColor,
      fontSize: fromNode.data.fontSize,
      borderStyle: fromNode.data.borderStyle,
      borderColor: fromNode.data.borderColor,
    } : {});
    pushHistory();
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      mr.onstop = async () => {
        ...
        fd.append("file", blob, `gravacao-${Date.now()}.webm`);
        try {
          const { data } = await api.post("/upload", fd, {
            headers: { "Content-Type": "multipart/form-data" },
          });
          const backendUrl = API_BASE.replace("/api", "");
          updateNodeData(selectedNodeId, { audio: { url: backendUrl + data.url, name: data.original_name } });
        } catch {
  ...
  const pasteNode = useCallback((atFlowPos) => {
    ...
    addNodeAt(pos.x, pos.y, {
      ...
      imageOpacity: clipboard.data.imageOpacity ?? null,
      video: clipboard.data.video || null,
      attachment: clipboard.data.attachment || null,
      audio: clipboard.data.audio || null,
      linkedMapId: clipboard.data.linkedMapId || null,
      startDate: clipboard.data.startDate || null,
      dueDate: clipboard.data.dueDate || null,
      textColor: clipboard.data.textColor || null,
      fontSize: clipboard.data.fontSize ?? null,
      table: clipboard.data.table || null,
      bgGradient: clipboard.data.bgGradient || null,
      gradientColor: clipboard.data.gradientColor || null,
      borderStyle: clipboard.data.borderStyle || null,
      borderColor: clipboard.data.borderColor || null,
      shadowSize: clipboard.data.shadowSize ?? null,
  ...
  const duplicateNode = useCallback((nodeId) => {
    ...
    addNodeAt(n.position.x + 40, n.position.y + 40, {
      ...
      imageOpacity: n.data.imageOpacity ?? null,
      video: n.data.video || null,
      attachment: n.data.attachment || null,
      audio: n.data.audio || null,
      linkedMapId: n.data.linkedMapId || null,
      startDate: n.data.startDate || null,
      dueDate: n.data.dueDate || null,
      textColor: n.data.textColor || null,
      fontSize: n.data.fontSize ?? null,
      table: n.data.table || null,
      bgGradient: n.data.bgGradient || null,
      gradientColor: n.data.gradientColor || null,
      borderStyle: n.data.borderStyle || null,
      borderColor: n.data.borderColor || null,
      shadowSize: n.data.shadowSize ?? null,
      ...
      const cleanedNodes = nodes.map(({ id: nid, position, data, type }) => ({
        ...
        position,
          data: {
            ...
            shadowSize: data.shadowSize ?? null,
  ...
  const previewUrl = selectedNodeId ? nodes.find((n) => n.id === selectedNodeId)?.data?.url : null;
  useEffect(() => {
    ...
    previewTimerRef.current = setTimeout(async () => {
      try {
        const { data } = await api.post("/preview", { url: previewUrl });
        setLinkPreview(data);
      } catch {
        setLinkPreview(null);
      } finally {
    ...
    <div className="h-screen w-screen flex flex-col bg-[#FDFBF7] overflow-hidden">
      ...
      <div className="flex-1 relative" ref={flowWrapper} data-testid="canvas-area" onMouseMove={onCanvasMouseMove}>
        ...
        <div
          ...
          className={`absolute left-4 top-4 z-20 flex flex-col gap-2 brutal-border brutal-shadow bg-white rounded-xl p-2 transition-all duration-200 ${toolbarCollapsed ? "w-auto" : ""}`}
        >
          <button
            onClick={() => { setToolbarCollapsed((v) => { const n = !v; localStorage.setItem("toolbarCollapsed", String(n)); return n; }); }}
            data-testid="toolbar-toggle"
            className="brutal-btn bg-white p-1 mb-1 self-center"
            title={toolbarCollapsed ? "Expandir barra" : "Recolher barra"}
          >
            {toolbarCollapsed ? <CaretRight size={14} weight="bold" /> : <CaretLeft size={14} weight="bold" />}
          </button>
          {!toolbarCollapsed && (
            <>
          <button
          ...
          <button
            onClick={() => { setInheritStyle((v) => { const n = !v; localStorage.setItem("inheritStyle", String(n)); return n; }); }}
            data-testid="inherit-style-toggle"
            className={`brutal-btn p-2 ${inheritStyle ? "bg-[#FDE047]" : "bg-white"}`}
            title="Herança de estilo: nós filhos herdam cor/forma do pai"
          >
          ...
          <button
            ...
            className={`brutal-btn p-2 ${showMiniMap ? "bg-[#FDE047]" : "bg-white"}`}
        ...
        {selectedNode && (
          ...
          >
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <label className="text-xs font-semibold text-gray-500">Tamanho:</label>
                <input
                  type="range"
                  min="8"
                  max="36"
                ...
                <span className="text-xs font-bold w-8 text-right">
                  {selectedNode.data.fontSize ?? 14}px
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              />
              {linkPreviewLoading && (
                <p className="text-[10px] text-gray-400 mt-1">Carregando preview...</p>
              )}
              {linkPreview?.title && (
                <div className="mt-2 flex gap-2 brutal-border rounded-md p-2 text-xs">
                  {linkPreview.image && (
                    <img src={linkPreview.image} alt="" className="w-10 h-10 object-cover rounded flex-shrink-0" onError={(e) => { e.target.style.display = "none"; }} />
                  )}
                  <div className="min-w-0">
                    <p className="font-bold truncate">{linkPreview.title}</p>
                    {linkPreview.description && <p className="text-gray-500 truncate">{linkPreview.description}</p>}
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <button
                  onClick={() => updateNodeData(selectedNode.id, { color: "transparent" })}
                  className={`w-8 h-8 brutal-border rounded-md flex items-center justify-center overflow-hidden ${
                    selectedNode.data.color === "transparent" ? "ring-4 ring-black/40" : ""
                  }`}
                  title="Transparente"
                  aria-label="cor transparente"
                >
                  <span className="w-full h-full relative">
                    <span className="absolute inset-0 bg-white" />
                    <span className="absolute inset-0 flex items-center justify-center">
                      <span className="w-full h-0.5 bg-red-400 rotate-45 origin-center" />
                    </span>
                  </span>
                </button>
                <input
                  type="color"
                  value={selectedNode.data.color === "transparent" ? "#FFFFFF" : selectedNode.data.color || "#FDE047"}
            ...
            {/* Gradiente */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Gradiente</p>
              <div className="flex items-center gap-2 mb-2">
                <label className="flex items-center gap-1 text-xs font-semibold cursor-pointer">
                  <input
                    type="checkbox"
                    checked={!!selectedNode.data.bgGradient}
                    onChange={(e) => updateNodeData(selectedNode.id, { bgGradient: e.target.checked ? "bottom right" : null, gradientColor: e.target.checked ? "#FFFFFF" : null })}
                    className="accent-black"
                  />
                  Ativar
                </label>
                {selectedNode.data.bgGradient && (
                  <>
                    <select
                      value={selectedNode.data.bgGradient}
                      onChange={(e) => updateNodeData(selectedNode.id, { bgGradient: e.target.value })}
                      className="flex-1 brutal-border rounded-md px-2 py-1 text-xs"
                    >
                      <option value="right">→ Direita</option>
                      <option value="left">← Esquerda</option>
                      <option value="bottom">↓ Baixo</option>
                      <option value="top">↑ Cima</option>
                      <option value="bottom right">↘ Diagonal</option>
                      <option value="bottom left">↙ Diagonal</option>
                      <option value="top right">↗ Diagonal</option>
                      <option value="top left">↖ Diagonal</option>
                    </select>
                    <input
                      type="color"
                      value={selectedNode.data.gradientColor || "#FFFFFF"}
                      onChange={(e) => updateNodeData(selectedNode.id, { gradientColor: e.target.value })}
                      className="w-8 h-8 brutal-border rounded-md cursor-pointer flex-shrink-0"
            ...
            {/* Borda */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Borda</p>
              <div className="flex gap-2 mb-2">
                {["solid", "dashed", "dotted", "double"].map((s) => (
                  <button
                    key={s}
                    onClick={() => updateNodeData(selectedNode.id, { borderStyle: s })}
                    className={`flex-1 brutal-border rounded-md py-1 text-xs font-bold uppercase ${
                      (selectedNode.data.borderStyle || "solid") === s ? "bg-[#FDE047]" : "bg-white"
                    }`}
                  >
                    {s === "solid" ? "Sólida" : s === "dashed" ? "Tracejada" : s === "dotted" ? "Pontos" : "Dupla"}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-2">
            ...
            {/* Sombra */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Sombra</p>
              <div className="flex items-center gap-2">
                <input
                  type="range"
                  min="0"
                  max="10"
                  value={selectedNode.data.shadowSize ?? 4}
                  onChange={(e) => updateNodeData(selectedNode.id, { shadowSize: Number(e.target.value) })}
                  className="flex-1 h-1.5 accent-black"
                />
                <span className="text-xs font-bold w-6 text-right">{selectedNode.data.shadowSize ?? 4}</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <input
                  ...
                  className="flex-1 brutal-border rounded-md px-3 py-2 text-sm"
                />
                <label className="brutal-btn bg-white px-3 py-2 text-xs font-bold cursor-pointer hover:bg-gray-100">
                  Upload
                  <input
                    type="file"
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        const backendUrl = API_BASE.replace("/api", "");
                        updateNodeData(selectedNode.id, { image: backendUrl + data.url });
                      } catch {
              ...
              {selectedNode.data.image && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <label className="text-xs font-semibold text-gray-500">Opacidade:</label>
                    <input
                      type="range"
                      min="0"
                      max="100"
                      value={Math.round((selectedNode.data.imageOpacity ?? 0.4) * 100)}
                      onChange={(e) => updateNodeData(selectedNode.id, { imageOpacity: Number(e.target.value) / 100 })}
                      className="flex-1 h-1.5 accent-black"
                    />
                    <span className="text-xs font-bold w-8 text-right">
                      {Math.round((selectedNode.data.imageOpacity ?? 0.4) * 100)}%
                    </span>
                  </div>
                  <div className="relative">
                    <img
                      ...
                      style={{ opacity: selectedNode.data.imageOpacity ?? 0.4 }}
                      onError={(e) => { e.target.style.display = "none"; }}
                    />
                    <button
                      onClick={() => updateNodeData(selectedNode.id, { image: null, imageOpacity: null })}
            ...
            {/* Vídeo */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Vídeo</p>
              <div className="flex items-center gap-2 mb-2">
                <input
                  value={selectedNode.data.video || ""}
                  onChange={(e) => updateNodeData(selectedNode.id, { video: e.target.value })}
                  placeholder="URL do vídeo (YouTube ou direta)..."
                  className="flex-1 brutal-border rounded-md px-3 py-2 text-sm"
                />
                <label className="brutal-btn bg-white px-3 py-2 text-xs font-bold cursor-pointer hover:bg-gray-100">
                  Upload
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        const backendUrl = API_BASE.replace("/api", "");
                        updateNodeData(selectedNode.id, { video: backendUrl + data.url });
                      } catch {
              ...
              {selectedNode.data.video && (
                <div className="mt-2 relative">
                  {selectedNode.data.video.includes("youtube.com") || selectedNode.data.video.includes("youtu.be") ? (
                    <div className="aspect-video rounded-md brutal-border overflow-hidden bg-black">
                      {(() => {
                        const m2 = selectedNode.data.video.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([\w-]+)/);
                        const embedSrc2 = m2 ? `https://www.youtube.com/embed/${m2[1]}` : selectedNode.data.video;
                        return (
                          <iframe
                            src={embedSrc2}
                    ...
                    <video
                      ...
                      controls
                      className="w-full h-28 object-cover rounded-md brutal-border"
                      onError={(e) => { e.target.style.display = "none"; }}
                    />
                  )}
                  <button
                    onClick={() => updateNodeData(selectedNode.id, { video: null })}
                    className="absolute -top-2 -right-2 brutal-border bg-white rounded-full w-5 h-5 flex items-center justify-center"
                  >
                    <X size={10} weight="bold" />
                  </button>
            ...
            {/* Anexo */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Anexo</p>
              {selectedNode.data.attachment ? (
                <div className="flex items-center gap-2">
                  <div className="flex-1 flex items-center gap-2 brutal-border rounded-md px-3 py-2 text-xs font-semibold truncate">
                    <Paperclip size={14} weight="bold" />
                    <span className="truncate flex-1">{selectedNode.data.attachment.name}</span>
                    <a
                      href={selectedNode.data.attachment.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="brutal-border rounded-md p-1 hover:bg-gray-100"
                      title="Download"
                    >
                      <FileArrowDown size={14} weight="bold" />
                    </a>
                    <button
                      onClick={() => updateNodeData(selectedNode.id, { attachment: null })}
                      className="brutal-border rounded-md p-1 hover:bg-gray-100"
                      title="Remover anexo"
                    >
                ...
                <label className="brutal-btn bg-white px-3 py-2 text-xs font-bold cursor-pointer hover:bg-gray-100 flex items-center gap-1">
                  <Paperclip size={14} weight="bold" />
                  Upload
                  <input
                    type="file"
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        updateNodeData(selectedNode.id, {
                          attachment: { name: data.original_name, url: backendUrl + data.url },
                        });
                      } catch {
            ...
            {/* Áudio */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Áudio</p>
              {selectedNode.data.audio ? (
                <div className="flex items-center gap-2">
                  <div className="flex-1 flex items-center gap-2 brutal-border rounded-md px-2 py-1">
                    <MusicNote size={14} weight="bold" className="flex-shrink-0" />
                    <audio src={selectedNode.data.audio.url} controls className="flex-1 h-8 min-w-0" />
                    <button
                      onClick={() => updateNodeData(selectedNode.id, { audio: null })}
                      className="brutal-border rounded-md p-0.5 hover:bg-gray-100 flex-shrink-0"
                      title="Remover áudio"
                    >
                      <X size={12} weight="bold" />
                    </button>
                ...
                <div className="flex gap-2">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          const backendUrl = API_BASE.replace("/api", "");
                          updateNodeData(selectedNode.id, {
                            audio: { url: backendUrl + data.url, name: data.original_name },
                          });
                        } catch {
                  ...
                  <button
                    ...
                    className={`brutal-btn px-3 py-2 text-xs font-bold flex items-center gap-1 ${
                      recording ? "bg-red-400 text-white" : "bg-white"
                    }`}
                    title={recording ? "Parar gravação" : "Gravar áudio"}
                  >
                    <Microphone size={14} weight={recording ? "fill" : "bold"} />
            ...
            {/* Mapa vinculado */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Mapa vinculado</p>
              {selectedNode.data.linkedMapId ? (
                <div className="flex items-center gap-2">
                  <div className="flex-1 flex items-center gap-2 brutal-border rounded-md px-3 py-2 text-xs font-semibold truncate">
                    <ArrowSquareOut size={14} weight="bold" />
                    <span className="truncate flex-1">
                      {userMaps.find((m) => m.id === selectedNode.data.linkedMapId)?.title || "Mapa"}
                    </span>
                    <a
                      href={`/editor/${selectedNode.data.linkedMapId}`}
                      onClick={(e) => { e.preventDefault(); navigate(`/editor/${selectedNode.data.linkedMapId}`); }}
                      className="brutal-border rounded-md p-1 hover:bg-gray-100"
                      title="Abrir mapa"
                    >
                      <ArrowSquareOut size={14} weight="bold" />
                    ...
                    <button
                      onClick={() => updateNodeData(selectedNode.id, { linkedMapId: null })}
                      className="brutal-border rounded-md p-1 hover:bg-gray-100"
                ...
                <select
                  value=""
                  onChange={(e) => updateNodeData(selectedNode.id, { linkedMapId: e.target.value || null })}
                  className="w-full brutal-border rounded-md px-3 py-2 text-xs"
                >
            ...
            {/* Datas */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Datas</p>
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <label className="text-xs font-semibold text-gray-500 w-12">Início:</label>
                  <input
                    type="date"
                    value={selectedNode.data.startDate || ""}
                    onChange={(e) => updateNodeData(selectedNode.id, { startDate: e.target.value || null })}
                    className="flex-1 brutal-border rounded-md px-2 py-1 text-xs"
                  />
                  {selectedNode.data.startDate && (
                    <button onClick={() => updateNodeData(selectedNode.id, { startDate: null })}>
                      <X size={12} weight="bold" />
                    </button>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-xs font-semibold text-gray-500 w-12">Entrega:</label>
                  <input
                    type="date"
                    value={selectedNode.data.dueDate || ""}
                    onChange={(e) => updateNodeData(selectedNode.id, { dueDate: e.target.value || null })}
                    className="flex-1 brutal-border rounded-md px-2 py-1 text-xs"
                  />
                  {selectedNode.data.dueDate && (
                    <button onClick={() => updateNodeData(selectedNode.id, { dueDate: null })}>
                      <X size={12} weight="bold" />
                    </button>
                  )}
                </div>
            ...
            <div>
              ...
              {selectedNode.data.table ? (
                <div className="space-y-2">
                  <div className="brutal-border rounded-md overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                      <thead>
                        <tr>
                          {selectedNode.data.table.headers.map((h, ci) => (
                            <th key={ci} className="brutal-border bg-gray-100 px-2 py-1 font-bold text-left">
                              <input
                                value={h}
                                onChange={(e) => {
                                  const t = { ...selectedNode.data.table };
                                  t.headers[ci] = e.target.value;
                                  updateNodeData(selectedNode.id, { table: t });
                                }}
                                className="w-full bg-transparent outline-none font-bold"
                              />
                            </th>
                          ))}
                          <th className="w-8 brutal-border bg-gray-100" />
                        </tr>
                      </thead>
                      <tbody>
                        {selectedNode.data.table.rows.map((row, ri) => (
                          <tr key={ri}>
                            {row.map((cell, ci) => (
                              <td key={ci} className="brutal-border px-2 py-1">
                                <input
                                  value={cell}
                                  onChange={(e) => {
                                    const t = { ...selectedNode.data.table };
                            ...
                            <td className="w-8 brutal-border px-1 py-1">
                              <button
                                onClick={() => {
                                  const t = { ...selectedNode.data.table };
                                  if (t.rows.length > 1) { t.rows.splice(ri, 1); updateNodeData(selectedNode.id, { table: t }); }
                                }}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="brutal-btn bg-white px-2 py-1 text-xs font-bold flex items-center gap-1"
                    >
                      <Plus size={12} weight="bold" /> Coluna
                    </button>
                    <button
                      onClick={() => {
                        const t = { ...selectedNode.data.table };
                      ...
                      className="brutal-btn bg-white px-2 py-1 text-xs font-bold flex items-center gap-1"
                    >
                      <Plus size={12} weight="bold" /> Linha
                    </button>
                    <button
                      onClick={() => updateNodeData(selectedNode.id, { table: null })}
                      className="brutal-btn bg-white px-2 py-1 text-xs font-bold text-red-500 ml-auto"
                    >
                      Remover
                    </button>
                  </div>
                ...
                <button
                  onClick={() => updateNodeData(selectedNode.id, { table: { headers: ["Item", "Valor"], rows: [["", ""]] } })}
                  className="brutal-btn bg-white px-3 py-2 text-xs font-bold w-full"
                >
                  + Adicionar tabela
                </button>
              )}
