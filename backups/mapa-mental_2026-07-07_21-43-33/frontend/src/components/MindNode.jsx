...
import { CaretDown, CaretRight, Link as LinkIcon, NotePencil, Plus, Minus, Paperclip, MusicNote, ArrowSquareOut } from "@phosphor-icons/react";
...
function MindNode({ id, data, selected }) {
  ...
  const daysRemaining = data.dueDate
    ? Math.ceil((new Date(data.dueDate) - new Date()) / (1000 * 60 * 60 * 24))
    : null;
  ...
  return (
    <div
      ...
      } ${data.dimmed ? "opacity-30" : ""} ${dragOver ? "ring-4 ring-[#93C5FD] bg-blue-50" : ""}`}
      style={{
        background: data.bgGradient && data.gradientColor
          ? `linear-gradient(to ${data.bgGradient}, ${color}, ${data.gradientColor})`
          : color,
        borderStyle: data.borderStyle && data.borderStyle !== "solid" ? data.borderStyle : undefined,
        borderColor: data.borderColor || undefined,
        boxShadow: data.shadowSize !== undefined ? `${data.shadowSize}px ${data.shadowSize}px 0px 0px rgba(0,0,0,1)` : undefined,
        width: nodeWidth,
        height: nodeHeight,
      }}
      ...
      onDrop={async (e) => {
        ...
        try {
          ...
          data.onChange?.(id, { image: backendUrl + uploadData.url });
    ...
    >
      ...
      {data.image && !data.video && (
        <img
          ...
          style={{ opacity: data.imageOpacity ?? 0.4 }}
          onError={(e) => { e.target.style.display = "none"; }}
        />
      )}

      {data.video && (
        <div className="absolute inset-0 w-full h-full rounded-[inherit] overflow-hidden">
          {data.video.includes("youtube.com") || data.video.includes("youtu.be") ? (
            (() => {
              const m = data.video.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([\w-]+)/);
              const embedSrc = m ? `https://www.youtube.com/embed/${m[1]}` : data.video;
              return (
                <iframe
                  src={embedSrc}
        ...
        {data.attachment && (
          <a
            ...
            className={`absolute -bottom-[30px] -left-[14px] z-30 brutal-border bg-white rounded-full w-7 h-7 flex items-center justify-center hover:bg-gray-100 ${isDiamond ? "-rotate-45" : ""}`}
            title={data.attachment.name}
          >
            <Paperclip size={14} weight="bold" className="text-black/70" />
          </a>
        )}
        {data.audio && (
          <span
            className={`absolute -bottom-[30px] left-[30px] z-30 brutal-border bg-white rounded-full w-7 h-7 flex items-center justify-center cursor-default ${isDiamond ? "-rotate-45" : ""}`}
            title={data.audio.name || "Áudio"}
          >
            <MusicNote size={14} weight="bold" className="text-black/70" />
          </span>
        )}
        {data.linkedMapId && (
          <a
            href={`/editor/${data.linkedMapId}`}
            onClick={(e) => { e.stopPropagation(); }}
            className={`absolute -top-[30px] left-[0px] z-30 brutal-border bg-white rounded-full w-7 h-7 flex items-center justify-center hover:bg-gray-100 ${isDiamond ? "-rotate-45" : ""}`}
            title="Abrir mapa vinculado"
          >
            <ArrowSquareOut size={14} weight="bold" className="text-black/70" />
          </a>
        )}
      ...
      {daysRemaining !== null && (
        <div className={`absolute bottom-0 left-0 right-0 h-1.5 rounded-b-[inherit] ${
          daysRemaining > 7 ? "bg-green-400" : daysRemaining >= 0 ? "bg-yellow-400" : "bg-red-400"
        }`} title={
