    ...
    >
      ...
      {editing ? (
        <input
          ...
          style={textStyle}
          className={`nodrag bg-transparent outline-none text-center text-black w-full ${isDiamond ? "-rotate-45" : ""}`}
          data-testid={`node-input-${id}`}
        />
      ) : (
        <span
          style={textStyle}
          className={`text-black text-center break-words px-1 ${isDiamond ? "-rotate-45" : ""}`}
        >
          {data.label || "Nó"}
        </span>
      )}
