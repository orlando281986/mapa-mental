...
function EditorInner() {
  ...
  const nodesForRender = useMemo(
    ...
    [nodes, commentCounts, childrenMap, descendantsMap, toggleCollapse, hiddenNodeIds],
  );
  ...
  }, [selectedNodeId, undo, redo, copyNode, cutNode, pasteNode, duplicateNode, deleteNode]);
  ...
  const handleContextAction = useCallback((action) => {
    ...
    switch (action) {
      ...
      case "delete": if (nid) deleteNode(nid); break;
      case "bringFront": if (nid) reorderNode(nid, "front"); break;
      case "sendBack": if (nid) reorderNode(nid, "back"); break;
      default: break;
    }
  }, [contextMenu, rf, copyNode, cutNode, pasteNode, duplicateNode, deleteNode, reorderNode, toggleCollapse]);
    ...
    <div className="h-screen w-screen flex flex-col bg-[#FDFBF7] overflow-hidden">
      ...
      <div className="flex-1 relative" ref={flowWrapper} data-testid="canvas-area" onMouseMove={onCanvasMouseMove}>
        <ReactFlow
          ...
          onPaneClick={() => { setSelectedNodeId(null); setContextMenu(null); setShowExportMenu(false); }}
