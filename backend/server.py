...
async def update_map(map_id: str, body: MindMapUpdate, user: dict = Depends(get_current_user)):
    ...
    await db_execute(
        ...
        (str(uuid.uuid4()), map_id, body.title or row["title"],
         json.dumps(body.nodes) if body.nodes else row["nodes"],
         json.dumps(body.edges) if body.edges else row["edges"], ts),
    )
...
if FRONTEND_BUILD.is_dir():
    ...
    logging.info("Frontend build found — serving SPA from %s", FRONTEND_BUILD)
else:
    logging.warning("Frontend build not found at %s — SPA fallback disabled", FRONTEND_BUILD)
